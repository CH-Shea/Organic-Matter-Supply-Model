...
Sim_Zoop_RealPhe( # use this if treating Phe as a conservative tracer
# Sim_Zoop(
    zoops.n = 7,
    PTS = 1,
    MTS = 1,
...
```{r posterior_trophic_comparison, dpi=300, fig.asp=0.3, fig.width=8.5}
...
library(effects)
ggplot() +
  geom_point(aes(y = posts.zoops$f$thin$Surface, x = posts.zoops$trophic$thin$MTS,
                 color = posts.zoops$f$thin$Size),
             shape=1, alpha=0.5, size=0.2)+
  geom_point(aes(y = posts.zoops$f$mode$Surface, x = posts.zoops$trophic$mode$MTS,
                 fill = posts.zoops$f$mode$Size),
             shape=21, size=2)+
  theme(legend.position = "bottom")
  
f_surface <- posts.zoops$f$mode$Surface
MTS <- posts.zoops$trophic$mode$MTS
# Size <- posts.zoops$trophic$mode$Size
model <- lm(f_surface ~ MTS)
