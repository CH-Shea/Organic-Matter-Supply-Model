---
title: "Organic Matter Supply Model: Analyzing data from wild zooplankton at Station ALOHA in winter"
author: "Connor Shea"
...
```{r export_filename}
export.file.name <- "OMSM_posteriors_ALOHA-Winter.RData"
```
...
Data.zoops <- 
  subset(Data.zoops,
         ((Depth < 50 | Depth > 200)&
            (Event == "Night" & Epoch == "Winter"))&
           d15NLys > min(Data.sources$d15NLys) & # removing sample with unrealistically low d15NLys value
           d15NThr > -24 & # removing chaetognath sample with unrealistic combination of thr and phe values
           Size != "0.06-0.2 mm" # removing smallest zooplankton size class
  )
...
plot_sourcepost(posts.long, Data.sources, Tracers)
...
noquote("EZ")
min(subset(posts$trophic$mode, Depth < 100)$PTS)
mean(subset(posts$trophic$mode, Depth < 100)$PTS)
max(subset(posts$trophic$mode, Depth < 100)$PTS)
noquote("MPZ")
min(subset(posts$trophic$mode, Depth > 200)$PTS)
mean(subset(posts$trophic$mode, Depth > 200)$PTS)
max(subset(posts$trophic$mode, Depth > 200)$PTS)
