...
```{r LDA_sources, dpi=300, fig.asp=0.8, eval=runMV}
if(runMV == TRUE){
...
```{r LDA_realzoops, dpi=300, fig.asp=0.8, eval=runMV}
if(runMV == TRUE){
...
samsPerChain <- 6000 # needed  below for narrative, so we give it a name.
...
AAplots <-
  ggplot(Sources.long[which(Sources.long$Tracer %in% Tracers$mix),])+
  geom_point(aes(x = Tracer, y = Value, color = Group),
             position = position_nudge(x = 0.2))+
  geom_point(data = Zoops.long[which(Zoops.long$Tracer %in% Tracers$mix),],
             aes(x = Tracer, y = Value, shape = Size # add shape = /VARIABLE/ to assign shapes to different types of consumers
             ),
             position = position_nudge(x = -0.2))+
...
library(effects)
posts.f.mode <- data.frame(
  Surface = posts.zoops$f$mode$Surface,
  FWL = posts.zoops$trophic$mode$FWL,
  Size = posts.zoops$f$mode$Size
)
posts.f.thin <- data.frame(
  Surface = posts.zoops$f$thin$Surface,
  FWL = posts.zoops$trophic$thin$FWL,
  Size = posts.zoops$f$thin$Size
)

ggplot(data = posts.f.mode) +
  geom_point(data = posts.f.thin,
    aes(y = Surface, x = FWL,
        color = Size),
    shape=1, alpha=0.5, size=0.5, show.legend = FALSE)+
  geom_point(
    aes(y = Surface, x = FWL, 
        fill = Size),
    shape=21, size=2)+
  geom_smooth(aes(y = Surface, x = FWL,
                  color = Size, fill = Size),
              method = "glm", method.args = list(family = quasibinomial()))+
  facet_wrap(~ Size, scales = "free")+
  theme(legend.position = "right", legend.direction = "vertical")+
  labs(fill = "Zooplankton Size")+
  guides(color = FALSE)+
  xlab("FWL")+
  ylab("f(surface)")
...
model <- glm(f_surface ~ MTS * Size, family = quasibinomial())
