...
This notebook is composed of the following sections:
  ...
  2. In the Source Separation section, multivariate analysis will be used to assess the extend to which sources of organic matter can be uniquely identified via the isotopic data supplied.
  3. In the Data Simulation section, a small number of consumer samples (here called zooplankton) will be synthesized for model testing and validation purposes.
  4. In the Model Execution section, the food web model will be fit and its performance assessed via simple diagnostics.
  5. In the Model Assessment section, the model's accuracy will be tested by comparing the model output to the known ecological parameters used to synthesize the consumer data.
...
```{r Tracers_Zoops.plots, dpi=300, fig.asp=0.5, fig.width=8.5}
Zoops.long <- melt(Data.zoops, id.vars=c(variables), measure.vars =c(Tracers$mix),
                     variable.name = "Tracer", value.name = "Value")
Base.long <- melt(base.sim, id.vars=c(variables), measure.vars =c(Tracers$mix),
                     variable.name = "Tracer", value.name = "Value")
...
```{r LDA_simzoops, dpi=300, fig.asp=0.8, eval=runMV}
if(runMV == TRUE){
...
fun_init_1 <- function(i) {
  inits <- 
    list(
      ...
      # sdFWL = rtgamma(n=N_Z, 0.001, 0.001, 10^-50), # SDFWL get and inverse gamma
      .RNG.seed = i+1,
...
```{r posterior_base_comparison, dpi=300, fig.asp=0.6, fig.width=8.5}
...
plot.FWL <-
  ...
  scale_linetype_manual(values=c(1,1))+geom_segment(
    aes(y=zoops.f$FWL, x=seq(1,nzoops)-0.5, xend=seq(1,nzoops)+0.5,
        color=""),
    lty=1,size=1,alpha=1)+
  # geom_hline(
  ...
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,3), xlim = c(1,nzoops))
plot.PTS <-
  ggplot()+
  # geom_violin(data = posts.zoops$trophic$samples,
  ...
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,2), xlim = c(1,nzoops))
plot.MTS <-
  ggplot()+
  # geom_violin(data = posts.zoops$trophic$samples,
...
# ggarrange(
#   ggarrange(
#     plot.MTS + theme(axis.title.x=element_blank(),axis.text.x=element_blank()),
#     plot.PTS + theme(axis.title.x=element_blank(),axis.text.x=element_blank()),
#     nrow=2, heights = c(1,1.1), legend = "none"),
#   plot.FWL + theme(axis.title.x=element_blank(),axis.text.x=element_blank()),
...
data.disc$Model <- "OMSM" # posts.zoops.mode$Model
data.disc$Model <- factor(data.disc$Model, levels = c("OMSM",
                                                      "MM(Phe,Lys)",
                                                      "MM(Ser,Gly,Phe,Lys)"))

data.disc.abs <- abs(data.disc[c("PTS","MTS","FWL")])
...
plot.TM.FWL <-
  ...
  geom_point(
    aes(x = true_value, y = model_value),
    size = 1, stroke=1, shape = 19, color = "steelblue4"
  )+
...
plot.Surface <-
  ...
  #   aes(x=Group, y=Surface, group = Group, fill=""),
  #   alpha=0.4, color="lightskyblue1", width=1)+
  scale_fill_manual(values=c("steelblue1","steelblue4"))+
  ...
  scale_color_manual(values=c("goldenrod"))+
  labs(fill="Model Posterior", color="True Value", lty="95% HDI")+
  xlab("Simulated Zooplankton Sample") + ylab("surface") +
  scale_y_continuous(breaks = c(0,0.25,0.5,0.75,1), labels = c("0","","0.5","","1.0"))+
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,1), xlim = c(1,nzoops))
plot.Large <-
  ggplot()+
  # geom_violin(data = posts.zoops$f$samples,
  #   aes(x=Group, y=Large, group = Group, fill=""),
  #   alpha=0.4, color="lightskyblue1", width=1)+
  scale_fill_manual(values=c("steelblue1","steelblue4"))+
  geom_line(data = posts.zoops$f$HDI95,
  ...
  xlab("Simulated Zooplankton Sample") + ylab("large") +
  scale_y_continuous(breaks = c(0,0.25,0.5,0.75,1), labels = c("0","","0.5","","1.0"))+
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,1), xlim = c(1,nzoops))
plot.Small <-
  ggplot()+
  # geom_violin(data = posts.zoops$f$samples,
  #   aes(x=Group, y=Small, group = Group, fill=""),
  #   alpha=0.4, color="lightskyblue1", width=1)+
  scale_fill_manual(values=c("steelblue1","steelblue4"))+
  geom_line(data = posts.zoops$f$HDI95,
  ...
  scale_color_manual(values=c("goldenrod"))+
  labs(fill="Model", color="Actual")+
  xlab("Simulated Zooplankton Sample") + ylab("small") +
  scale_y_continuous(breaks = c(0,0.25,0.5,0.75,1), labels = c("0","","0.5","","1.0"))+
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,1), xlim = c(1,nzoops))
plot.SurfaceLarge <-
  ...
  scale_color_manual(values=c("goldenrod"))+
  labs(fill="Model Posterior", color="True Value", lty="95% HDI")+
  xlab("Simulated Zooplankton Sample") + ylab("surface + large") +
  scale_y_continuous(breaks = c(0,0.25,0.5,0.75,1), labels = c("0","","0.5","","1.0"))+
  scale_x_continuous(breaks = seq(1,nzoops))+
  coord_cartesian(ylim = c(0,1), xlim = c(1,nzoops))
  ...
  #   plot.SurfaceLarge + no.x.axis,
  #   nrow = 4, ncol = 1, heights = c(1,1,1,1.1), legend = "none", common.legend = TRUE)
...
data.disc.abs <- abs(data.disc[c("Surface","Large","Small","SurfaceLarge")])
data.disc.abs$Model <- data.disc$Model
...
```{r mix-troph-post-summary, fig.width=6.5, fig.asp=1, dpi=300}

ggarrange(
  
  ggplot()+
    ggtitle("Mixing Parameter Posteriors"),
  
  ggarrange(
    plot.Surface + no.x.axis,
      plot.TM.surf + no.x.axis + no.y.axis,
    plot.Large + no.x.axis,
      plot.TM.large + no.x.axis + no.y.axis,
    plot.Small + theme(axis.title.x = element_blank()),
      plot.TM.small + theme(axis.title.x = element_blank()) + no.y.axis,
    nrow = 3, ncol = 2, heights = c(1,1,1.1), widths = c(4,1), legend = "none"),
  
  ggplot()+
    ggtitle("Trophic Parameter Posteriors"),
  ...
  ggarrange(
    ...
    ggarrange(
      plot.PTS + no.x.axis,
        plot.TM.PTS + no.x.axis + no.y.axis,
      plot.MTS + no.x.axis,
        plot.TM.MTS + no.x.axis + no.y.axis,
      plot.FWL,
        plot.TM.FWL + no.y.axis + xlab("True Value"),
      nrow=3, ncol=2, heights = c(2,2,4), widths = c(4,1),
      legend="none"),
    ncol = 2, widths = c(0.018,1)
  ),
  
  nrow = 4, heights = c(0.1,1,0.1,1)
...
```{r f-vs-TP, fig.width=8, fig.asp=0.6, dpi=300}
## FWL ##
ggplot() +
  geom_point(aes(x = posts.zoops$f$thin$Surface, y = posts.zoops$trophic$thin$MTS,
                 color = posts.zoops$f$thin$Size),
             shape=1, alpha=0.5, size=0.2)+
  geom_point(aes(x = posts.zoops$f$mode$Surface, y = posts.zoops$trophic$mode$MTS,
                 fill = posts.zoops$f$mode$Size),
             shape=21, size=2)+
  geom_smooth(aes(x = posts.zoops$f$mode$Surface, y = posts.zoops$trophic$mode$MTS),
...
## PTS ##
ggplot() +
  geom_point(aes(x = posts.zoops$f$thin$Surface, y = posts.zoops$trophic$thin$PTS,
                 color = posts.zoops$f$thin$Size),
             shape=1, alpha=0.5, size=0.2)+
  geom_point(aes(x = posts.zoops$f$mode$Surface, y = posts.zoops$trophic$mode$PTS,
                 fill = posts.zoops$f$mode$Size),
             shape=21, size=2)+
  geom_smooth(aes(x = posts.zoops$f$mode$Surface, y = posts.zoops$trophic$mode$PTS),
              method = "glm", method.args = list(family = gaussian()))+
  theme(legend.position = "bottom")+
  xlab("f(surface)")+
  ylab("PTS")
...
## FWL ##
ggplot() +
  geom_point(aes(y = posts.zoops$f$thin$Surface, x = posts.zoops$trophic$thin$FWL),
             shape=1, alpha=0.5, size=0.2)+
  geom_point(aes(y = posts.zoops$f$mode$Surface, x = posts.zoops$trophic$mode$FWL,
                 fill = as.factor(zoops.f$Surface)),
             shape=21, size=2)+
  geom_smooth(aes(y = posts.zoops$f$mode$Surface, x = posts.zoops$trophic$mode$FWL,
                  color = as.factor(zoops.f$Surface), fill = as.factor(zoops.f$Surface)),
              method = "glm", method.args = list(family = gaussian()))+
