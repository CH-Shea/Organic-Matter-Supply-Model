---
title: "Organic Matter Supply Model: Assessing model performance using simulated data from Station ALOHA"
author: "Connor Shea"
...
```{r export_filename}
export.file.name <- "ALOHAsim_OMSM_frac-Phe_posteriors.RData"
```

## Identifying Tracers {-}
Next we'll identify tracers to be included in the model and some key parameters on how to treat them. Here we'll be using $\dN$ values of Phe, Lys, and Thr. All tracers are assumed to fractionate during trophic transfer.
...
# Comparing to model results without fractionation in Phe {-}

Now we'd like to build a couple simple plots illustrating how including/excluding fractionation in Phe impacts our ability to accurately solve the mixing problem.
...
mix.disc$Model <- "OMSM - fractionating Phe"
mix.disc$Model <- factor(mix.disc$Model, levels = c("OMSM - conservative Phe","OMSM - fractionating Phe"))
...
mix.disc.full$Model <- "OMSM - conservative Phe" # posts.mode$Model
mix.disc.full$Model <- factor(mix.disc.full$Model, levels = c("OMSM - conservative Phe","OMSM - fractionating Phe"))
...
plot.TM.surf <-
  ...
  geom_smooth(data = subset(data.truemod.long, Source == "Surface"),
    aes(x = f_true, y = f_model, color = "Fractionating Phe"),
        fill = "salmon",
    # method = "lm"
    method = "glm", method.args = list(family = binomial())
  )+
  geom_smooth(data = subset(data.truemod.full.long, Source == "Surface"),
    aes(x = f_true, y = f_model, color = "Conservative Phe"),
        fill = "skyblue1",
    # method = "lm"
    method = "glm", method.args = list(family = binomial())
  )+
...
plot.disc.surf <-
  ggplot()+
  geom_smooth(data = subset(data.truemod.long, Source == "Surface"),
    aes(x = f_true, y = f_disc, color = "Fractionating Phe"),
        fill = "salmon",
    method = "lm"
    # method = "glm", method.args = list(family = binomial())
  )+
  geom_smooth(data = subset(data.truemod.full.long, Source == "Surface"),
    aes(x = f_true, y = f_disc, color = "Conservative Phe"),
        fill = "skyblue1",
    method = "lm"
    # method = "glm", method.args = list(family = binomial())
  )+
