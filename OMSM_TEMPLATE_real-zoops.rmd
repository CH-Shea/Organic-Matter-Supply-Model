...
In the first few chunks, the user should enter all of the basic information that the model code will need to proceed with analysis. Lines of code requiring user attention will be indicated with `#### USER ATTENTION REQUIRED ####`.
...
- One column containing a unique identifier for each sample
...
```{r export_filename}
########################## USER ATTENTION REQUIRED #############################
export.file.name <- "OMSM_TEMPLATE_sim-zoops.Rdata" #  indicate the name of the file you would like to export model results to (must end in .Rdata)
################################################################################
```
...
```{r Process_Data, eval=TRUE, fig.asp=0.6, out.width="100%"}
...
if(FALSE){
  ...
  # see OMSM_ALOHA-Summer_real-zoops.rmd for examples
  Data.sources <- subset(Data.sources, !is.na(Source))
}

#### Isolating Consumer Data ####
# Consumer data will be stored in a separate data frame
Data.zoops <- 
  subset(Data.all, Type == "Consumer")
########################## USER ATTENTION REQUIRED #############################
#### Programatically filtering consumer samples ####
## would you like to programatically filter the consumer samples?
## if so set to TRUE
if(FALSE){
  # construct criteria to filter consumer samples and overwrite Data.zoops 
  # data frame.
  # see OMSM_ALOHA-Summer_real-zoops.rmd for examples
  Data.zoops <-
    subset(Data.zoops)
...
Now let's generate plots of the MCMC posteriors for the zooplankton sample-specific parameters. To start, we'll want to plot the model posteriors for the tracer values at the base of the food web. We'll do this for all the individual tracers, but also try and visualize the multivariate patterns using LDA. The LDA doesn't include mixing tracers.
...
We also want to plot up the model posteriors for trophic parameters. The chunks below plot full posterior distributions for each sample. See OMSM_ALOHA-Summer_real-zoops.rmd for additional plotting schemes.
...
The following chunk can be used to compute summary statistics or linear models based on model outputs like distribution modes, means, or HDIs. `posts` and `posts.long` are large lists containing convenient forms of model outputs. `posts` contains all data frames in wide format, while `posts.long` contains all data frames in long format. The first level lists all posterior output types.

  - posts$trophic contains trophic parameters.
  - posts$f contains mixing parameters.
  - posts$source contains tracer values of organic matter sources.
  - posts$base contains tracer values at the base of the food web.
  - posts$zoops contains tracer values in zooplankton.
  
The second level lists contain different representations of model posteriors.

  - samples contains all "samples" from the MCMC chain for each consumer sample and represents the full posterior PDF.
  - thin contains a thinned version of the samples list.
  - mean and mode contain the mean and mode of the posterior.
  - HDINN contain the max and min values of 50%, 75%, 90%, and 95% HDIs of the posteriors.
  

```{r summary-stats}

noquote("Contents of posts:")
str(posts, max.level = 2)
noquote("Contents of posts.long:")
str(posts.long, max.level = 2)
