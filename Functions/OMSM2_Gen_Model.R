...
OMSM_Trophic_Parameters <-
  "      ## Carrying out trophic enrichment from food web base to zooplankton
      ...
      MTS[iz] <- (Y[iz,i_T_MTS] - mean_b[iz,i_T_MTS]) * TDF_meta[i_T_MTS]^-1
      FWL[iz] <- (Y[iz,i_T_FWL] - mean_b[iz,i_T_FWL]) * TDF_meta[i_T_FWL]^-1
      PTS[iz] <- FWL[iz] - MTS[iz]
      sdPTS[iz] <- sqrt(sdFWL[iz]^2 + sdMTS[iz]^2)
"
