...
Sim_Zoop_RealPheLys( # use this if treating Phe as a conservative tracer in the model
...
Base.long <- melt(base.sim, id.vars = variables, measure.vars = c(Tracers$MTS, Tracers$FWL, Tracers$mix),
                  variable.name = "Tracer", value.name = "Value")
Sources.long <- melt(Data.sources, id.vars = variables, measure.vars = c(Tracers$MTS, Tracers$FWL, Tracers$mix),
                   variable.name = "Tracer", value.name = "Value")
AA_plots <- ggplot(Sources.long) +
  geom_point(aes(x = Tracer, y = Value, color = Group, shape = Group), alpha = 0.6,
...
if (runMV) {
  plot.LDA <- TRUE # Set to false if PCA plot is preferred
  if (plot.LDA == TRUE) {
    ...
    pred.train <- predict.PCA(PCA, Data.sources[Tracers$mix], scale.unit = FALSE
    )
    class.train <- data.frame(pred.train$coord,
                              lapply(Data.sources[variables], as.character))
    
    pred.zoops <- predict.PCA(PCA, Data.zoops[Tracers$mix], scale.unit = FALSE
    )
    class.zoops <- data.frame(pred.zoops$coord,
                              lapply(Data.zoops[variables], as.character))
    pred.base <- predict(PCA, base.sim[Tracers$mix], scale.unit = FALSE)
    class.base <- data.frame(pred.base$coord,
                             lapply(base.sim[variables], as.character))
  }
  
  plot.mix.LD12 <- 
    ## If using PCA remember to change LD1->Dim.1 and LD2->Dim.2
    ggplot(class.train, aes(x = LD2, y = LD1, color = Group, shape = Group)) +
...
troph.disc$Model <- "OMSM - no Thr" # posts.zoops.mode$Model
troph.disc$Model <- factor(troph.disc$Model, levels = c("OMSM - no Thr",
                                                      "OMSM - full"))
...
mix.disc$Model <- "OMSM - full" # posts.zoops.mode$Model
...
Now we'll do a quick check for systematic error in the model residuals.

```{r f-vs-TP, fig.width=4, fig.asp=1.5, dpi=300}
  
# Systematic error in trophic parameters
...
# Systematic Error in mixing coefficients

model <- lm(mix.disc$Surface ~ posts.zoops$trophic$mode$FWL)
# plot(allEffects(mod=model))
anova(model)
  
model <- lm(mix.disc$Large ~ posts.zoops$trophic$mode$FWL)
# plot(allEffects(mod=model))
anova(model)

model <- lm(mix.disc$Small ~ posts.zoops$trophic$mode$FWL)
# plot(allEffects(mod=model))
anova(model)
...
plot.disc.fsurf <-
  ggplot() +
  geom_smooth(aes(y = mix.disc$Surface, x = posts.zoops$trophic$mode$FWL),
              method = "lm",
              color = "royalblue4", fill = "skyblue1")+
  geom_point(aes(y = mix.disc$Surface, x = posts.zoops$trophic$mode$FWL),
             shape = 21 ,size=2, fill = "steelblue4")+
  theme(legend.position = "none")+
  coord_cartesian(ylim = c(-1,1), xlim = c(1,3), expand = FALSE)+
  ylab("f(surface)")+
...
plot.disc.flarge <-
  ggplot() +
  geom_smooth(aes(y = mix.disc$Large, x = posts.zoops$trophic$mode$FWL),
              method = "lm",
              color = "royalblue4", fill = "skyblue1")+
  geom_point(aes(y = mix.disc$Large, x = posts.zoops$trophic$mode$FWL),
             shape = 21 ,size=2, fill = "steelblue4")+
  theme(legend.position = "none")+
  coord_cartesian(ylim = c(-1,1), xlim = c(1,3), expand = FALSE)+
  ylab("f(large)")+
...
plot.disc.fsmall <-
  ggplot() +
  geom_smooth(aes(y = mix.disc$Small, x = posts.zoops$trophic$mode$FWL),
              method = "lm",
              color = "royalblue4", fill = "skyblue1")+
  geom_point(aes(y = mix.disc$Small, x = posts.zoops$trophic$mode$FWL),
             shape = 21 ,size=2, fill = "steelblue4")+
  theme(legend.position = "none")+
  coord_cartesian(ylim = c(-1,1), xlim = c(1,3), expand = FALSE)+
  ylab("f(small)")+
...
ggarrange(
  ggplot()+
    ...
    theme(plot.title = element_text(angle = 90)),
  ggarrange(
    plot.disc.fsurf + no.x.axis,
    plot.disc.flarge + no.x.axis,
    plot.disc.fsmall,
    ncol=1, heights = c(1,1,1.25)
  ),
  ncol = 2, widths = c(0.1,1)
...
The last thing we want to do here is export out mixing model results to a .xlsx file.

```{r export-results, eval=TRUE}
save(posts.global, posts.global.long, 
     posts.zoops, posts.zoops.long, 
     Data.all, zoops.sim, zoops.f,
     file = "Data/OSPsim_OMSM1_posteriors_no-Thr.RData")
```
...
load("Data/OSPsim_OMSM1_posteriors.RData")
posts.OMSM2.zoops <- posts.zoops
posts.OMSM2.zoops.long <- posts.zoops.long

# load("Data/OSPsim_OMSM1_posteriors_no-Thr.RData")
# posts.OMSM2.zoops <- posts.zoops
# posts.OMSM2.zoops.long <- posts.zoops.long
...
data.model.full <- 
  cbind(
    posts.OMSM2.zoops$f$mode[c("Surface","Large","Small","SurfaceLarge")]
  )

mix.disc.full <- data.model.full-data.true
mix.disc.full$Model <- "OMSM - no Thr" # posts.zoops.mode$Model
mix.disc.full$Model <- factor(mix.disc.full$Model, levels = c("OMSM - full","OMSM - no Thr"))

mix.disc.full.abs <- abs(mix.disc.full[c("Surface","Large","Small")])
mix.disc.full.abs$Model <- mix.disc.full$Model

mix.disc.full.sum <- aggregate(.~Model,
                            data=mix.disc.full.abs,
                            FUN=sum)
mix.disc.full.sum

mix.disc.full.mean <- aggregate(.~Model,
                            data=mix.disc.full.abs,
                            FUN=mean)
mix.disc.full.mean

print("max absolute discrepancy - Surface")
max(mix.disc.full.abs$Surface)
print("max absolute discrepancy - Large")
max(mix.disc.full.abs$Large)
print("max absolute discrepancy - Small")
max(mix.disc.full.abs$Small)
...
plot.TM.surf <-
  ...
  geom_smooth(data = subset(data.truemod.long, Source == "Surface"),
    aes(x = f_true, y = f_model, color = "Non-Fractionating"),
        fill = "salmon",
    # method = "lm"
    method = "glm", method.args = list(family = binomial())
  )+
...
plot.disc.surf <-
  ggplot()+
  geom_smooth(data = subset(data.truemod.long, Source == "Surface"),
    aes(x = f_true, y = f_disc, color = "Non-Fractionating"),
        fill = "salmon",
    method = "lm"
    # method = "glm", method.args = list(family = binomial())
  )+
