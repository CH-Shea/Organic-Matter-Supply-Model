...
## defining aditional functions we might need
...
Source.Variable <- "NAME OF SOURCE VARIABLE"
...
Sources <- c("NAMES OF ORGANIC MATTER SOURCE GROUPS")
...
Consumers <- c("NAMES OF CONSUMER GROUPS")
...
Descriptive.Variables <- c("NAMES OF DESCRIPTIVE VARIABLES")
...
Tracers.constTDF <- c("TRACER1","TRACER2","...")
SDTracers.constTDF <- c("SDTRACER1","SDTRACER2","...")
...
Tracers.exclude <- c("TRACER2","...")
SDTracers.exclude <- c("SDTRACER2","...")
...
Data.all <- read_excel("DATA FILE LOCATION.xlsx",
...
Data.Sources <-
  subset(Data.Sources) # add subsetting conditions (e.g., Depth > 200 & Size == "Small")
...
# Further subsetting data....
Data.zoops <- 
  subset(Data.Zoops) # add subsetting conditions (e.g., Depth > 200 & Size == "Small")
# If a sample is missing data it will be removed in this step
...
Sources.long <- melt(Data.Sources, id.vars=c(variables), measure.vars =c(Tracers.all),
...
Zoops.long <- melt(Data.zoops, id.vars=c(variables), measure.vars =c(Tracers.all),
                     variable.name = "Tracer", value.name = "Value")
...
AA_plots <-
  ...
  geom_point(data = Zoops.long,
             aes(x = Tracer, y = Value, shape="Zooplankton" # add shape = /VARIABLE/ to assign shapes to different types of consumers
             ),
             position = position_nudge(x = -0.1))+
  ylab("Value")+
...
Sources.alpha <- c("X_A","X_B","X_C","X_D","X_E","X_F")[1:N_S]
SDSources.alpha <- c("sdX_A","sdX_B","sdX_C","sdX_D","sdX_E","sdX_F")[1:N_S]
nsams_alpha   <- c("N_A","N_B","N_C","N_D","N_E","N_F")[1:N_S]
...
means_alpha <- c("mean_A","mean_B","mean_C","mean_D","mean_E","mean_F")[1:N_S] # names for organic matter sources
sd_alpha <- c("sd_A","sd_B","sd_C","sd_D","sd_E","sd_F")[1:N_S] # and their uncertainty
  ...
  inits <- 
    list(
      ...
      TDF_proto = runif(length(TDF_proto[Tracers.frac]), 
                        ...
                        as.double(TDF_proto[Tracers.frac]+TDF_proto[SDTracers.frac])),
      pz        = rdirichlet(n=N_Z, c(1,1,1,1)), # mixing coefficients can take any starting composition
...
AAplots <-
  ...
  geom_point(data = Zoops.long,
             aes(x = Tracer, y = Value, fill="zooplankton" # add shape = /VARIABLE/ to assign shapes to different types of consumers
                 ), pch = "triangle",
             position = position_nudge(x = -0.3))+
